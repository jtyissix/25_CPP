...
MyGame3* MyGame3::currentInstance = nullptr;

MyGame3::MyGame3(QWidget *parent) : QMainWindow(parent) {
    currentInstance = this;
    if (parent && parent->inherits("Intro")) {
        parentIntro = parent;
    }
    battleScene = new BattleScene3(this);
    view = new QGraphicsView(this);
    view->setScene(battleScene);
    view->setFixedSize((int) view->scene()->width(), (int) view->scene()->height());
    view->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    view->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    connect(battleScene, &BattleScene3::gameOver, this, &MyGame3::onGameOver);
...
void MyGame3::onGameOver(const QString& winner) {
    if (battleScene) {
    ...
    if (result == QMessageBox::Yes) {
        QTimer::singleShot(100, []() { MyGame3::restartGame(); });
    } else {
        close();
    }
}

void MyGame3::restartGame() {
    qDebug() << "开始重启游戏...";
    MyGame3* oldInstance = currentInstance;
    MyGame3* newGame = new MyGame3();
    newGame->show();
    if (oldInstance) {
...
void MyGame3::closeEvent(QCloseEvent *event) {
    emit windowClosed();
    event->accept();
}

void MyGame3::createPauseDialog() {
...
QPushButton* MyGame3::createCapsuleButton(const QString& text, const QColor& color) {
...
void MyGame3::keyPressEvent(QKeyEvent *event) {
